...
> **專案名稱**：L01_AI基本功  
> **專案路徑**：`D:\Antigravity\L01_AI基本功`  
> **協作角色**：高工資訊科老師 × 電子工程師 × 教務主任  
> **規範依據**：遵照 `D:\Antigravity\AGENTS.md` 第 7、8 條款標準維護。
...
- **執行內容**：
  1. 檢視並更新 `D:\Antigravity\AGENTS.md`，新增第 7 章「檔案分類與目錄架構規範」與第 8 章「專案歷程記錄規範」。
  2. 建立專案專屬歷程文件 `HISTORY.md`，完整回溯至今的所有工作歷程與技術細節。
  3. 規劃專案標準分流目錄：`docs/`（教材文件）、`subtitles/`（逐字稿）、`downloads/`（媒體大檔）、`scripts/`（自動化工具）、`prompts/`（模型規範）。
- **異動與產出檔案**：
  - `D:\Antigravity\AGENTS.md`（修改：新增 Section 7 & 8）
  - `D:\Antigravity\L01_AI基本功\HISTORY.md`（新建：本歷程紀錄檔）
...
- **工作背景**：建立 `D:\Antigravity` 全域與子專案協作基礎。
- **執行內容**：
  1. 確立使用者 4 合 1 專業身分：電子工程師、軟體工程師、高工資訊科老師、教務主任。
  2. 建立 `D:\Antigravity\AGENTS.md` 全域協作規範與子專案 `Agents.md`。
  3. 透過 Winget 安裝 `yt-dlp`、`FFmpeg`、`Deno` 等工具鏈環境。
  4. 設定 `.gitignore` 排除 `downloads/`、媒體暫存檔與 `.env`。
- **異動與產出檔案**：
  - `D:\Antigravity\AGENTS.md`
  - `D:\Antigravity\L01_AI基本功\Agents.md`
  - `D:\Antigravity\L01_AI基本功\.gitignore`
