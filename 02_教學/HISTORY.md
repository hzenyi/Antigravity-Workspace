# 教學領域歷程記錄 (02_教學 / HISTORY.md)

本文件依據全域協作規範，嚴謹記錄 `02_教學` 專案之重大里程碑、教材生成、影片影音資源與重要產出。

---

## [2026-09-13] 教學工作目錄正名與子目錄繁體中文化

- **任務背景**：
  配合工作區全域三大分類架構（01_行政、02_教學、03_程式），將原 `A02_教學工作` 正式更名為 `02_教學`，並落實全目錄繁體中文化。
- **執行內容與技術關鍵**：
  1. 子目錄繁體中文化更名：
     - `_共用範本庫_Templates/` $\rightarrow$ `共用範本庫/`
     - `subtitles/` $\rightarrow$ `字幕逐字稿/`
     - `tools/` $\rightarrow$ `教學工具/`
     - `scripts/` $\rightarrow$ `工具腳本/`
  2. 重構更新 `02_教學/AGENTS.md`，將暫存目錄全面更新為 `暫存草稿/`，更新相對參照路徑。

---

## [2026-09-05] 教學工作專案協作規範升級為 Pedagogical Harness Engines 架構

- **任務背景**：
  因應使用者建構 Harness Engines 體系需求，全面檢視全域根目錄規範（`G:\我的雲端硬碟\91_Antigravity\AGENTS.md`）與子專案目錄規範（`G:\我的雲端硬碟\91_Antigravity\A02_教學工作\AGENTS.md`）之角色重複性，並將教學專案升級為具備任務路由、外骨骼治具、不可逾越防護紅線與品質審核閘門之智慧治理基座。
- **執行內容與技術關鍵**：
  1. **消除角色重疊與理順繼承體系**：
     - 明確宣告子專案繼承全域「高工資訊科老師」為核心主持人，調用全域「電子工程師」與「專業程式設計師」為專業學科領域專家 (Domain SMEs)。
     - 精簡原先與全域重複的教育態度說教，將教學分工提升解耦為「四大教學驅動引擎 (Engines)」。
  2. **建置教學工作智慧治理外骨骼 (Pedagogical Harness)**：
     - **學科與任務路由 (Task Router)**：依輸入自動分流學科領域並派工至對應引擎（E1 教案規劃、E2 教材講義、E3 評量設計、E4 批改診斷）。
     - **工作階段暫存治具 (Workspace Scratchpad)**：新增各學科 `_working_/` 暫存區（素材萃取、草稿自修復、批改暫存），避免半成品污染正式歸檔區。
     - **不可逾越之教學防護紅線 (Redline Guardrails)**：設立學生個資代號化保護、做中學階梯式引導（嚴禁直接餵代碼解答）、教材溯源與課本頁碼圖號對照、正式題庫安全防護四大硬性阻斷條件。
     - **教學品質評審閘門與自修復回路 (Review Gates & Loop)**：產出成果前強制執行課綱審、教材審、量規審、批改審、隱私審等五大閘門，未通過自動迭代重試（上限 3 次）。
     - **統一交付格式與審計追蹤 (Delivery & Audit Trace)**：規範結構化成果交付卡片，並強制登錄至 `HISTORY.md`。
  3. **同步更新專案說明 (README.md)**：
     - 更新專案分類架構樹狀圖，納入 `_working_/` 暫存區與 Harness Engines 運作機制說明。
- **異動與產出檔案清單**：
  - [MODIFY] `G:\我的雲端硬碟\91_Antigravity\A02_教學工作\AGENTS.md` (重構為 Pedagogical Harness Engines 架構)
  - [MODIFY] `G:\我的雲端硬碟\91_Antigravity\A02_教學工作\README.md` (同步專案分類架構與基座說明)
  - [MODIFY] `G:\我的雲端硬碟\91_Antigravity\A02_教學工作\HISTORY.md` (登錄本期歷程記錄)
- **驗證狀態**：
  - 各檔案語法格式檢驗通過，Markdown 語法標號與代碼塊完整閉合，成功與全域規範及 A01 基座規範保持一致之嚴謹水準。
...
- **執行內容與技術關鍵**：
  1. **全域安全與版本控制規則建立**：
     - 於根目錄 `G:\我的雲端硬碟\91_Antigravity` 建立完整的 `.gitignore`，嚴格排除敏感個資、金鑰密碼 (`.env`)、影音下載暫存 (`downloads/`)、Python 快取 (`__pycache__/`) 與編輯器暫存。
  2. **自動化備份與匯入工具開發**：
