...
- **任務背景**：
  因應使用者建構 Harness Engines 體系需求，全面檢視全域根目錄規範（`d:\Antigravity\AGENTS.md`）與子專案目錄規範（`d:\Antigravity\A02_教學工作\AGENTS.md`）之角色重複性，並將教學專案升級為具備任務路由、外骨骼治具、不可逾越防護紅線與品質審核閘門之智慧治理基座。
- **執行內容與技術關鍵**：
  1. **消除角色重疊與理順繼承體系**：
...
- **異動與產出檔案清單**：
  - [MODIFY] `d:\Antigravity\A02_教學工作\AGENTS.md` (重構為 Pedagogical Harness Engines 架構)
  - [MODIFY] `d:\Antigravity\A02_教學工作\README.md` (同步專案分類架構與基座說明)
  - [MODIFY] `d:\Antigravity\A02_教學工作\HISTORY.md` (登錄本期歷程記錄)
- **驗證狀態**：
